...
flash_boot() {
  ...
  # Use real partition size from sysfs instead of comparing with original boot.img
  ak3_bname=$(basename $BLOCK 2>/dev/null);
  if [ -n "$ak3_bname" ] && [ -f "/sys/class/block/$ak3_bname/size" ]; then
    ak3_partsize=$(cat /sys/class/block/$ak3_bname/size 2>/dev/null);
    ak3_partsize=$((ak3_partsize * 512));
    ak3_newsize=$(wc -c < boot-new.img);
    ui_print "- Partition: $BLOCK (/dev/block/$ak3_bname)" >&2;
    ui_print "- Partition size: $ak3_partsize bytes" >&2;
    ui_print "- New boot-new.img: $ak3_newsize bytes" >&2;
    if [ "$ak3_newsize" -gt "$ak3_partsize" ]; then
      abort "New image ($ak3_newsize) larger than partition ($ak3_partsize). Aborting...";
    fi;
