...
/* ksu_cred is defined in KernelSU; referenced by susfs_run_extra_works */
...
/* ============ SukiSU-Ultra compat stubs ============ */
/* susfs_extra_works: required by SukiSU-Ultra lsm_hook.c */
struct work_struct susfs_extra_works;
static void susfs_run_extra_works(struct work_struct *work) {
    if (!ksu_cred)
        return;
    /* v1.5.5 does not have susfs_run_sus_path_loop() */
}
/* This function is referenced in lsm_hook.c via extern, so it must be non-static */
struct work_struct *susfs_get_extra_works(void) { return &susfs_extra_works; }

/* These functions provide the API expected by SukiSU-Ultra's dispatch.c
 * while using the v1.5.5 SUSFS mechanism internally. Some are stubs. */

#ifdef CONFIG_KSU_SUSFS_SUS_PATH
void susfs_add_sus_path_loop(void __user **user_info) {
    /* stub: sus_path_loop is a v2.x feature, not implemented in v1.5.5 */
    SUSFS_LOGI("susfs_add_sus_path_loop: stub (not supported in v1.5.5)\n");
}
#endif

#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
void susfs_set_hide_sus_mnts_for_non_su_procs(void __user **user_info) {
    /* stub: hide_sus_mnts_for_non_su_procs is a v2.x feature */
    SUSFS_LOGI("susfs_set_hide_sus_mnts_for_non_su_procs: stub\n");
}
#endif
...
void susfs_add_sus_map(void __user **user_info) {
    /* stub: sus_map is a v2.x feature */
    SUSFS_LOGI("susfs_add_sus_map: stub (not supported in v1.5.5)\n");
}
#endif

void susfs_set_avc_log_spoofing(void __user **user_info) {
    /* stub: avc_log_spoofing is a v2.x feature */
    SUSFS_LOGI("susfs_set_avc_log_spoofing: stub\n");
}
...
void susfs_get_enabled_features(void __user **user_info) {
    ...
    int len = 0;
#ifdef CONFIG_KSU_SUSFS_SUS_PATH
    len += scnprintf(info.enabled_features + len, SUSFS_ENABLED_FEATURES_SIZE - len, "sus_path ");
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_MOUNT
    len += scnprintf(info.enabled_features + len, SUSFS_ENABLED_FEATURES_SIZE - len, "sus_mount ");
#endif
#ifdef CONFIG_KSU_SUSFS_SUS_KSTAT
    len += scnprintf(info.enabled_features + len, SUSFS_ENABLED_FEATURES_SIZE - len, "sus_kstat ");
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_UNAME
    len += scnprintf(info.enabled_features + len, SUSFS_ENABLED_FEATURES_SIZE - len, "spoof_uname ");
#endif
#ifdef CONFIG_KSU_SUSFS_SPOOF_CMDLINE_OR_BOOTCONFIG
    len += scnprintf(info.enabled_features + len, SUSFS_ENABLED_FEATURES_SIZE - len, "spoof_cmdline ");
#endif
#ifdef CONFIG_KSU_SUSFS_OPEN_REDIRECT
    len += scnprintf(info.enabled_features + len, SUSFS_ENABLED_FEATURES_SIZE - len, "open_redirect ");
#endif
    info.err = 0;
...
void susfs_show_variant(void __user **user_info) {
    ...
    scnprintf(info.susfs_variant, SUSFS_MAX_VARIANT_BUFSIZE - 1, "%s", SUSFS_VARIANT);
    info.err = 0;
out:
...
void susfs_show_version(void __user **user_info) {
    ...
    scnprintf(info.susfs_version, SUSFS_MAX_VERSION_BUFSIZE - 1, "%s", SUSFS_VERSION);
    info.err = 0;
out:
...
void susfs_start_sdcard_monitor_fn(void) {
    /* stub: sdcard monitor is a v2.x feature */
    SUSFS_LOGI("susfs_start_sdcard_monitor_fn: stub\n");
}
